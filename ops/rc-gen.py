...
def write_run_dir(row, workdir, turn_budget=0, bugfix_lua_errors=True,
                   bugfix_indefinite_transform=True, bugfix_spell_mana_check=True):
    ...
    bugfix_spell_mana_check (default True) controls docs/decisions/014's
    QW_BUGFIX_SPELL_MANA_CHECK rc flag — the Phase 2 A/B toggle for the
    plans-spells.lua inverted mana-affordability check (blocked casting
    exactly when affordable). False reproduces the original inverted
    check, for an experiment's control arm.
    ...
    rc_text = (RC_TMPL_PATH.read_text()
               ...
               .replace("__BUGFIX_SPELL_MANA_CHECK__",
                        "true" if bugfix_spell_mana_check else "false"))
...
def main():
    ...
    if args.workdir:
        layout = write_run_dir(row, args.workdir, turn_budget=args.turn_budget,
                                ...
                                bugfix_spell_mana_check=
                                not args.disable_bugfix_spell_mana_check)
