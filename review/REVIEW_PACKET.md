# Review packet: PLAN.md (dcss-automation, PR #1)

You are reviewing a planning document for a DCSS automation project. Your
environment has no GitHub or web access, so this packet is self-contained:

- **Section 1** — the review task.
- **Section 2** — the full text of `PLAN.md` (the document under review; it is
  the only file in the PR, added at the repository root on branch
  `claude/dcss-bot-planning-o5e03o`).
- **Section 3** — a verification ledger: which factual claims in the document
  were already verified against primary sources by the authoring session, how,
  and which remain unverified. Treat "verified" entries as trustworthy inputs
  you do not need to re-check; focus your skepticism on the "unverified"
  entries and on the document's *reasoning*, which no ledger can vouch for.

The base repository contains only a two-line README; PLAN.md is greenfield
planning, not a change to existing code.

---

## 1. Review task

Review the document as a senior engineer familiar with game automation and
autonomous-agent systems. Evaluate:

1. **Technical feasibility** of the proposed architecture for DCSS.
2. **Randomness**: is the proposed character generation genuinely unbiased
   with respect to the stated goal ("completely random character")? Identify
   any gap between the goal statement and the recommended defaults.
3. **Observation fairness**: does the proposed observation interface avoid
   hidden-information leakage (i.e., does the bot only act on
   player-knowable state)?
4. **Actionability** of the milestones, phase exit criteria, and evaluation
   methodology (metrics, A/B comparisons, seeding).
5. **Unsupported assumptions, missing risks, unnecessary complexity**, and
   places needing concrete implementation detail or upstream verification.

Deliver findings ordered by severity with exact line references into the
PLAN.md text below (line numbers are given in Section 2's fence header
comment; the file is reproduced verbatim), then suggest focused edits. Do not
assume you can modify anything — report only.

---

## 2. PLAN.md (verbatim, 308 lines)
...
## 3. Verification ledger (from the authoring session)

The authoring session verified facts by cloning `github.com/crawl/qw` and
`github.com/elliptic/qw` and reading their source directly, and by fetching
files from `github.com/crawl/crawl` (raw source) and public docs. Status of
the document's load-bearing claims:

**Verified directly against qw source (cloned, files read):**
- Repo structure: 43 Lua modules (~19k lines), `qw.rc`, `make-qw.sh`
  concatenation build, plan-cascade engine in `plans.lua`, coroutine main
  loop driven from `ready()` in `source/main.lua`.
- `AUTO_START` (qw.rc line 29, default false), `QUIT_TURNS = 1000`
  (qw.rc line 103), `COMBO_CYCLE_LIST` with `SpBg.weapon^gods` syntax
  (qw.rc lines 35–39).
- `util/hypercombogen.sh` exists and invokes `crawl -list-combos`, assigning
  starting weapons via sed rules (Fi/Gl→waraxe-class, Mf→trident, Sp→rapier,
  Fe/Tr→claws, etc.) — i.e., its weapon assignment is curated, not random.
- `util/batch-qw.sh` runs parallel instances in tmux with independent crawl
  dirs and deletes `logfile`/`milestones`/save files with `-c`.
- qw version coupling: tags 0.1.0→DCSS 0.29, 0.2.0→0.30, 0.3.0→0.31,
  master→0.32-a0; last public commit July 2024.
- God support list, goal language, and the "Normal" route as described.

**Verified against crawl source (fetched files):**
- clua modules `l-you.cc`, `l-moninf.cc`, `l-view.cc`, `l-item.cc`,
  `l-crawl.cc`: read API is player-knowledge-limited (monster damage given in
  bands, unseen monsters return nil, item info identification-gated);
  `crawl.sendkeys` / `process_keys` / `do_commands` exist.
- `newgame.cc`: menu keys `*` (random), `+` (recommended random), `!` (fully
  random character), `#` (recommended random character).
- `docs/options_guide.txt`: `fully_random`, `combo`, multi-value
  `species`/`background` lists, `viable` keyword, `name` bypassing the menu.
- `main.cc` help text: `-name`, `-species`, `-background`, `-rc`, `-seed`,
  `-wizard`, `-headless`, `-lua-max-memory`, `-throttle`/`-no-throttle`.
  **Note: `-list-combos` is NOT in the help text** — it demonstrably exists
  (hypercombogen calls it) but is undocumented.
- `hiscores.cc`: score formula terms as summarized (declining XP multipliers,
  rune terms, +250k win bonus). Minor additional terms exist and were not
  fully extracted.
- Webtiles protocol details (zlib framing, `msgs` batching, message types)
  from `webserver/` and `tileweb.cc`.
- Species/background counts: 27 species and 26 backgrounds counted from
  `dat/species/*.yaml` and `dat/jobs/*.yaml` on **trunk/master** (not on the
  0.32 tag — see unverified items).

**Reported by research but not independently re-verified (treat with
skepticism):**
- qw's historical win rates (~15% DDFi^Makhleb offline; ~1% 15-rune
  GrFi^Oka) — from the elliptic-era README and an arXiv paper citing it; the
  maintained repo's own accomplishments file lists "best 15-rune attempt:
  12 runes" for its most recent dated attempt, so the 15-rune-win claim
  rests on the maintained README's summary sentence plus elliptic-era
  history.
- 0.31-tournament results (first Dj/Dg wins, 2024) — via search snippets of
  crawl.develz.org, which was not directly fetchable.
- "qw finishes games in minutes of wall-clock" — inference, not measured.
- Whether a plain local (non-DGL) crawl build writes the `milestones` file —
  suggested by batch-qw.sh referencing it, but not confirmed in crawl's
  build defines.
- Whether qw master actually runs against current crawl trunk (vs. only
  0.32-a0).
- Exact species/background pool of the 0.32-a0 tag (trunk counts were used
  in the document; 0.32 lacks Poltergeist/Revenant and retains Armataur, so
  the pool differs from the "current game" description).
- BALROG/NetHack/Pokémon LLM-agent results — from papers/press summaries,
  reasonable confidence but secondhand.
